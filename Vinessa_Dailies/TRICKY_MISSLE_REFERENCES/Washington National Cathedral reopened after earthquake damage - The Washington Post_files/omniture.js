...
function s_doPlugins(s) {
	...
	*/
	...
	wp_defaultEvents = (self === top) ? 'event1' : 'event32' ;
 ...
 */
s.measurementPlugin=new Function("Z","B","N","Y",""
+"var s=this,r,t,f,D,E,P,F,Q,G,H,j=Z+'&',c,d,l,S,C,T,U='',M,O=s._o2d,"
+"V,W,X,z='prop,eVar,hier',q,p='channel,server,products,purchaseID,zi"
+"p,state,pageType';Y=Y?Y:'12/31/2050';j=B?j+'ns_site='+B+'&':j;j=s_a"
+"ccount?j+'rsid='+s_account+'&':j;X=new Date();Y=new Date(Y);if(X<Y)"
+"{function R(x,o,n){var i=x.indexOf(o),l=n.length;while(x&&i>=0){x=x"
+".substring(0,i)+n+x.substring(i+o.length);i=x.indexOf(o,i+l)}return"
+" x}function A(u,v){var y,x=0,w=new Array;while(u){y=u.indexOf(v);y="
+"y>-1?y:u.length;w[x++]=u.substring(0,y);u=u.substring(y+v.length)}r"
+"eturn w}if(s.linkType=='o'){G='o';H=s.linkName}else{F=s.p_gh();if(F"
+"){F=F.indexOf('?')>-1?F.substring(0,F.indexOf('?')):F;E=A(s.linkInt"
+"ernalFilters,',');for(Q=0;Q<E.length;Q++){if(F.indexOf(E[Q])>-1){W="
+"1}}G=W==1?G:'e';I=A(F,'/');P=A(s.linkDownloadFileTypes,',');for(Q=0"
+";Q<P.length;Q++){if(I&&I[I.length-1].indexOf('.'+P[Q])>-1)G='d'}if("
+"G)H=F}}if(O==1||G){j=s.pageName?j+'name='+s.pageName:j+'name='+s.wd"
+".location;l=A(z,',');q=A(p,',');if(O==1){for(var i in s){if(typeof "
+"s[i]!='undefined'&&s[i]){for(c=0;c<3;c++){if(i.indexOf(l[c])>-1){d="
+"s[i]+'';V=R(i,'prop','p');V=R(V,'eVar','e');j=j+'&'+V+'='+d}}for(c="
+"0;c<7;c++){if(i==q[c]){j=j+'&'+i+'='+s[i]}}if(i=='events')j=j+'&'+i"
+"+'='+s[i]+','}}}else if(G!='undefined'){j=j+'&lt='+G;j=j+'&ln='+H;j"
+"=j+'&ns_type=hidden';r=String(s.linkTrackVars);t=String(s.linkTrack"
+"Events);f=String(s.events);l=A(r,',');S=A(t,',');T=A(f,',');s.event"
+"s='';for(c=0;c<S.length;c++){for(C=0;C<T.length;C++){if(S[c]==T[C])"
+"{U=U+','+T[C]}}}s.events=U.substring(1,U.length);for(var i in s){if"
+"(typeof s[i]!='undefined'&&s[i]){for(c=0;c<8;c++){if(i==l[c]){d=s[i"
+"]+'';V=R(i,'prop','p');V=R(V,'eVar','e');j=j+'&'+V+'='+d}}}}}M=N==1"
+"?'&visid='+s.c_r('s_vi'):'';j=j.toLowerCase()+M;s.udm_('http'+(docu"
+"ment.location.href.charAt(4)=='s'?'s://sb':'://b')+'.scorecardresea"
+"rch.com/b?c1=2&c2='+j);s._o2d=0}}");s._o2d=1;
s.udm_=new Function("a",""
...
if (typeof(wp_site_identity) != "undefined")
{
...
s.eVar58=(typeof(wp_evar58)!="undefined")?wp_evar58:'';
s.eVar59=(typeof(wp_evar59)!="undefined")?wp_evar59:'';
s.eVar60=(typeof(wp_evar60)!="undefined")?wp_evar60:'';
...
s.nullifyOnResend = ['events','campaign','prop20','prop27','eVar3','eVar13','eVar20'];
...
s.sendPageViewToComscoreDontPanic = function(){
	if (!!window.console&&!!console.log) {
		console.log("DON'T PANIC: Comscore will be called on the live site.");
	}
}
s.sendPageViewToComscore = function(){
	...
	try{
		...
		if( !!window.location.hostname.match(/\.washingtonpost\.com/) ){
			pvcUrl = "/wp-srv/comscore/pageview_candidate.txt";
			var i = new Image();
			i.onload = i.onerror = function(){
				jQuery.ajax(
					pvcUrl,
					{dataType:"text",cache:false,error:function(){}}
				);
			}
			i.src = (("https:" == document.location.protocol) ? "https://sb" : "http://b")
				+ ".scorecardresearch.com/b?c1=2&c2=3005617"
				+ "&c7="+encodeURI(jQuery("meta[property='og:url']").attr("content")||location.href)
			;
		} else {
...
function waitForPaywallTracking (setTimer,checkCount) {
		checkCount +=1;	
		window.console && console.log && console.log("OMNITURE - WAIT FOR PAYWALL: " + setTimer);
		window.console && console.log && console.log("OMNITURE - CHECKCOUNT: " + checkCount);
		if ((s && TWP && TWP.Identity && TWP.Identity.paywall && TWP.Identity.paywall.pwresp) || checkCount > 6 || setTimer != true) {
			// initialize lots of variables
			s.copyTWPToWd();
			s.initForTwp();
			if (self === top) {
			...
			window.console && console.log && console.log("OMNITURE - PAYWALL VARIABLES: 64:" + s.eVar64 + " 65: " + s.eVar65 + " 66: " + s.eVar66);			
		} else {
			setTimeout(function(){waitForPaywallTracking(true,checkCount)},500);
		}			 
}

if (typeof _suppressTrackPageLoad == 'undefined' || !_suppressTrackPageLoad ) {
	if (window.TWP && 
		TWP.Identity && 
		TWP.Identity.paywall && 
		TWP.Identity.paywall.requireTracking === true) {
			$(document).ready(function(){
				setTimeout(function(){waitForPaywallTracking(true,checkCount=0)},500);	
			});		
	}	else {
