...
(function (win, doc, undefined) {
  ...
  wpAd = {
    ...
    postscribe: {
      ...
      init: function(){
        wpAd.tools.ajax({
          url: 'http://js.washingtonpost.com/wp-srv/ad/postscribe.js',
          cache: true,
          dataType: 'script',
          timeout: 2000,
          crossDomain: true,
      ...
      on_load: function(){
        if(window.jQuery){
          window.jQuery(document).ready(function(){
            wpAd.postscribe.doPostscribe();
          });
        } else {
          // This timeout attempts to fix issues with wordpress pages:
          setTimeout(function(){
            wpAd.postscribe.doPostscribe();
          }, 500);
        }
    ...
    exec: {
      ...
      adi: function () {
        ...
        if(slug) {
          ...
          iframe = wpAd.tools.iframeBuilder({
            "id": "ad_iframe_" + pos,
            "name": "ad_iframe_" + pos,
          ...
          if(briefcase.adi_insert_before && slug.firstChild){
            slug.insertBefore(iframe, slug.firstChild);
          } else {
            slug.appendChild(iframe);
          }
          iframe.src = briefcase.dcUrl;
    ...
    tools: {
      ...
      addKruxScript: function(dataID){
        ...
        var krux_script = doc.createElement('script'),
          target = doc.getElementsByTagName('head')[0] || doc.body;
        krux_script.src = 'http://js.washingtonpost.com/wp-srv/ad/krux.js';
        krux_script.type = 'text/javascript';
        krux_script.async = true;
        krux_script.className = 'kxct';
        krux_script.setAttribute('data-id', dataID);
      ...
      demoAds: function () {
        ...
        while(l--) {
          ...
          if(/\*$/.test(demoAds[l])){
            demoAds[l] = demoAds[l].split(/\*$/)[0];
            openAll = true;
          }

          if(demoAds[l] === 'tiffanytile' && wpAd.constants && wpAd.constants.site === 'wpni'){
            obj.tiffany_tile = openAll ? {openAll: true} : {};
            obj.tiffany_tile_2 = openAll ? {openAll: true} : {};
          } else {
            obj[wpAd.tools.convertOldPosValue(demoAds[l])] = openAll ? {openAll: true} : {};
          }
      ...
      // REQUIRES JQUERY - loop through all iframe ads on the page, if it doesn't have an id or name, reload it:
      iframeAdReloader: function(){
        $('[id^="slug_"] iframe:visible').each(function(el, i){
          var $this = $(this), src;
          if(!$this.attr('id') || !$this.attr('name')){
            //src = $this.attr('src');
            //$this.attr('src', src);
            wpAd.tools.addPixel('http://ad.doubleclick.net/N701/ad/wpni.test/iframetest;sz=1x1;ord=[TIMESTAMP]?');
            if(window.console){
              window.console.log('Reloaded iframe:', $this[0], src)
            }
          }
        });
      },
    ...
    keyvalues: {
        ...
        return wpAd.cache.hasOwnProperty('exclusion') ? wpAd.cache.exclusion : (function () {
          var rv = [],
            obj = {
              ...
              human_disaster : ['shoot', 'vatican', 'spanair', 'aground', 'rescue', 'attack', 'disaster', 'explosion', 'war', 'hostage', 'terror', 'terrorist', 'bomb', 'blast', 'mining', 'miner', 'violence', 'riot', 'crash', '9/11', 'sept. 11', 'september 11', 'behead', 'decapitate', 'derailment', 'tamerlan', 'tzhokhar', 'tsarnaev', 'jahar', 'rape', 'rapist', 'syria'],
      ...
      ref: function () {
        return wpAd.cache.hasOwnProperty('ref') ? wpAd.cache.ref : (function () {
          wpAd.cache.ref = [];
          var d = doc.referrer || '';
          if(/digg\.com|facebook\.com|reddit\.com|myspace\.com|newstrust\.net|twitter\.com|delicious\.com|stumbleupon\.com|linkedin\.com|tumblr\.com|pinterest\.com|plus\.google\.com|klout\.com|youtube\.com|bebo\.com|tagged\.com|orkut\.com|badoo\.com/i.test(d)) {
      ...
      viewable: function(){
        //21458-CD
        return wpAd.briefcase.delivery === 'vi' ? ['true'] : [];
      },
      //Krux kv's begin
      khost: function(){
  ...
  wpAd.flags = {
    ...
    reloadIframes: !!/reloadIframes/i.test(location.search)
  };
