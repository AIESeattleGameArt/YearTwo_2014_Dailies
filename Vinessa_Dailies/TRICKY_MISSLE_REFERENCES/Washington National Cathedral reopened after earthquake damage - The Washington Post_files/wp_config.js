wpAd.config.templates = {
  ...
  //default templates
  default01: { what: ['enterprise', 'leaderboard', 'leaderboard_2', 'leaderboard_grid', 'flex', 'flex_re', 'flex_bb_hp', 'flex_ss_bb', 'flex_ss_tp', 'flex_ss_bb_hp', '120x240', '200x50', '150x60', '285x29', 'bigbox', 'bigbox_vi', 'inline_bb*', 'inline_lb*', 'itb', 'skyscraper', 'grid_bigbox*', 'persistent_bb'] },
  ...
  subscribed_promo: {
    ...
    when: ['201307100000/201312312359'],
    test: [function(){
      if(/show-subscribed-promo/.test(location.search)){
        return true;
      }
      //subscribed: TWP.Identity.paywall.pwresp.sub = 1
      return window.TWP && window.TWP.Identity && window.TWP.Identity.paywall &&
        window.TWP.Identity.paywall.pwresp && window.TWP.Identity.paywall.pwresp.sub &&
        parseInt(window.TWP.Identity.paywall.pwresp.sub, 10) === 1 || false;
    }],
    hardcode: function(){
      var container = document.createElement('div');
      if(window.jQuery){
        window.jQuery('#wpni_adi_pushdown').removeClass('slug');
        window.jQuery('<img src="http://img.wpdigital.net/wp-srv/ad/img/welcome-1-970x66.png" height="66" width="970" alt="" style="border:0;outline:0;" />').appendTo(container);
      }
      return container;
    }
  },
  ...
  subscribe_promo: {
    ...
    hardcode: function() {
      var container = document.createElement('div'),
        $ = window.jQuery,
        creative = '<a href="https://account.washingtonpost.com/acquisition/?promo=dgbanrad&destination&tid=signup_HP5">' +
          ...
          '</a>';
        //creative = '<img src="//img.wpdigital.net/wp-adv/advertisers/marketing/meter/site-currently-free-970x70.png" alt="washingtonpost.com is currently free" width="970" height="66" style="border:0;outline:0;" />';

      if($){
        $('#wpni_adi_pushdown').removeClass('slug');
        $(container).attr({
          id: 'wpad_promo_banner_container'
        }).append(creative);
      }
      return container;
    },
    test: [function() {
      ...
      var TWPVal,
          TWP = window.TWP || {};
      if (TWP.Identity && TWP.Identity.paywall && TWP.Identity.paywall.pwresp && TWP.Identity.paywall.pwresp.mtfn === 1 && TWP.Identity.paywall.pwresp.mtr === 1 && TWP.Identity.paywall.pwresp.sub === 0) {
        TWPVal = true;
      } else if (window.commercialNode === "washingtonpost.com" && TWP.Identity && TWP.Identity.paywall && TWP.Identity.paywall.pwresp && TWP.Identity.paywall.pwresp.mtfn === 1 && TWP.Identity.paywall.pwresp.mtr === 20 && TWP.Identity.paywall.pwresp.sub === 0){
        TWPVal = true;
      } else {
        TWPVal = false;
      }
